...
Route::group(['middleware' => ['auth'], 'as' => 'dashboard.', 'prefix' => 'dashboard'], function(){
    ...
    Route::post('/users/set-not-verified/{id}', [UserController::class, 'setNotVerified'])->name('users.setNotVerified');
    ...
    Route::get('reports/change-status/{id}', [ReportController::class, 'showChangeStatus'])->name('reports.showChangeStatus');
    Route::patch('reports/change-status/{id}', [ReportController::class, 'changeStatus'])->name('reports.changeStatus');
    Route::get('/users/detail/{id}', [UserController::class, 'detail'])->name('users.detail');
});
Auth::routes(['verify' => true]);

require __DIR__.'/print-reports.php';
