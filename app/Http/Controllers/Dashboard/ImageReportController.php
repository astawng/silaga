...
class ImageReportController extends Controller
{
    ...
    public function index($id = null)
    {
        ...
        $images = ImageReport::where('report_id', $report->report_id)->paginate(10);
    ...
    public function store(Request $request)
    {
        ...
        if($request->filename)
        {
            $fileindex = Report::where('report_id', $request->report_id)->firstOrFail();
    ...
    public function show($id = null, $type)
    {
        ...
        if($type == 'show')
        {
            $image = ImageReport::where('image_report_id', $id)->firstOrFail();
            $report = Report::where('report_id', $image->report_id)->firstOrFail();
            $data = [
                'image' => $image,
                'report' => $report
            ];
