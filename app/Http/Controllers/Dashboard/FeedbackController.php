...
class FeedbackController extends Controller
{
    ...
    {
        $reportChecker = Report::where('report_id', $request->report_id)->first();
        if(!$reportChecker || $reportChecker?->status != 'done')
        {
            return back()->with('error', 'Report tidak ditemukan');
        }
        // Generate kode unik FDBxxx
        $last = Feedback::orderBy('feedback_id', 'desc')->first();
        if ($last && preg_match('/^FDB(\d{3})$/', $last->feedback_id, $matches)) {
            $num = (int)$matches[1] + 1;
        } else {
            $num = 1;
        }
        $newId = 'FDB' . str_pad($num, 3, '0', STR_PAD_LEFT);
        $data = $request->all();
        $data['feedback_id'] = $newId;
